...
SE2Error ErrorEstimator::estimate_error(const TargetEdge &target_edge) {
  ...
  */
  ...
  // 가장 안정적인 수직축(normal_axis)을 기준으로 무조건 왼쪽 90도 회전시킨 고정 가짜 축을 하나 만듭니다.
  Eigen::Vector2f consistent_lat_axis(-target_edge.normal_axis.y(), target_edge.normal_axis.x());
  ...
  RCLCPP_INFO(this->get_logger(), "y_error: %f", y_error);
  RCLCPP_INFO(this->get_logger(), "theta_error: %f", theta_error);
